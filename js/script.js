(function () {
  ...
  var cart = JSON.parse(localStorage.getItem('cb_cart')) || [];
  ...
  function renderCart() {
    ...
    if (cart.length === 0) {
      ...
      if (totalEl) totalEl.textContent = 'GHS 0';
      if (checkoutEl) {
    ...
    if (checkoutEl) {
      ...
      checkoutEl.textContent = 'Proceed to Checkout';
      checkoutEl.href = 'checkout.html';
      // Optional: keep the WhatsApp link as a secondary option if desired, but for now we follow the plan to use checkout.html
      checkoutEl.removeAttribute('target'); // Open in same tab or new tab as per preference, usually same tab for checkout flow
    }
    ...
    container.innerHTML = cart.map(function (item, index) {
      ...
      return (
        ...
        '<span class="cart-item-price">GHS ' + (item.price * item.qty) + '</span>' +
        '</div>' +
    ...
    if (totalEl) totalEl.textContent = 'GHS ' + totalGhc;
  ...
  function initCart() {
    ...
    // Removed the preventDefault for empty cart on checkout click here because renderCart handles hiding it.
